...
path_C = os.environ["PATH_subtypes_main"]
path_data = os.environ["PATH_SNIa_data"]
sys.path.insert(1,path_C+'codes/modules/')
...
class L_grid(object):
    ...
    def __init__(self, line_mode='downbranch', left_panel='11fe', show_pEW=False, show_fig=True, save_fig=False):
        ...
        self.fs_label, self.fs_ticks, self.fs_legend, self.fs_text,	self.fs_as, self.fs_feature	= ut.font_size_for_plotting()
	
        self.run_comparison()
		
    def set_fig_frame(self):
        self.ax1.set_xlabel(r'$\mathrm{rest \ wavelength} \ \mathrm{[\AA}]}$',fontsize=self.fs_label)
        self.ax1.set_ylabel(r'$\mathrm{Relative \ f}_{\lambda}$',fontsize=self.fs_label)
        ...
        self.ax2.set_xlabel(r'$\mathrm{rest \ wavelength} \ \mathrm{[\AA}]}$',fontsize=self.fs_label)
        #self.ax2.set_ylabel(r'$\mathrm{Relative \ f}_{\lambda}$',fontsize=self.fs_label)
    ...
    def load_spectra(self):
        ...
        """
        Load pkl files for the luminosity grid.
        """	
        ...
        for L in self.L_array:																		
            with open(path_data+'synthetic_spectra/'+self.left_panel+'_Lgrid_'+folder_appendix+'/loglum:'+str(format(np.log10(L), '.2f'))+'_v7.pkl', 'r') as inp:
